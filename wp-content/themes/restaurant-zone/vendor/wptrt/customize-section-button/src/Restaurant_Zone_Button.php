...
class Restaurant_Zone_Button extends WP_Customize_Section {
	...
	public function json() {
		...
		if ( ! $this->button_url && $restaurant_zone_theme->get( 'ThemeURI' ) ) {

			$button_url = $restaurant_zone_theme->get( 'ThemeURI' );

		// Fall back to the `Author URI` defined in `style.css`.
		} elseif ( ! $this->button_url && $restaurant_zone_theme->get( 'AuthorURI' ) ) {

			$button_url = $restaurant_zone_theme->get( 'AuthorURI' );
		}

		$json['button_text'] = $this->button_text ? $this->button_text : $restaurant_zone_theme->get( 'Name' );
